...
def filterNonParticipators(theFrame):
 ...
 while icount<len(theFrame['Community Name']):
  try:
   ...
   Community=str(theFrame["Community Name"][icount]).replace("40s","").replace("40'","").replace("45s","").replace("|","").relace("/","").replace("&"," ")\
   ...
   .replace("85'","").replace("90s","").replace("90'","").replace("95s","").replace("95'","").replace("100s","").replace("100'","")
   """
   .replace("105s","")\
   ...
   .replace("Austin_TX>269>Highpointe /_>146097","Austin_TX>269>Highpointe_>146097");
   """
